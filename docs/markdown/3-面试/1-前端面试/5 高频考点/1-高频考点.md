...
可以从上述代码中发现，不管我们给函数 `bind` 几次， `fn` 中的 `this` 永远由第⼀次 `bind` 决定，所以结果永远是 `window`
...
`bind` 的实现对比其他两个函数略微地复杂了⼀点， 因为 `bind` 需要返回⼀个函数， 需要判断⼀些边界问题， 以下是 `bind` 的实现
...
总的来说性能优化这个领域的很多内容都很碎片化， 这⼀章节我们将来学习这些碎片化的内容
...
在 `Webpack3` 中， 我们⼀般使用 `UglifyJS` 来压缩代码，但是这个是单线程运行的， 为了加快效率， 我们可以使用 `webpack-parallel-uglify-plugin` 来并行运行 `UglifyJS` ，从而提高效率。
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101632875.png)

但是 `MVC` 有一个巨大的缺陷就是控制器承担的责任太大了，随着项目愈加复杂，控制器中的代码会越来越臃肿，导致出现不利于维护的情况。

在 `MVVM` 架构中，引入了 `ViewModel` `的概念。ViewModel` 只关心数据和业务的处理，不关心 `View` 如何处理数据，在这种情况下， `View` 和 `Model` 都可以独立出来，任何一方改变了也不一定需要改变另一方，并且可以将一些可复用的逻辑放在一个 `ViewModel` 中，让多个 `View` 复用这个 `ViewModel` 。

![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101634506.png)

以 `Vue` 框架来举例， `ViewModel` 就是组件的实例。 `View` 就是模板， `Model` 的话在引入 `Vuex` 的情况下是完全可以和组件分离的。

除了以上三个部分，其实在 `MVVM` 中还引入了一个隐式的 `Binder` 层，实现了 `View` 和 `ViewModel` 的绑定。

![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101635089.png)
...
另外如果你使用 `Vue2.3` 及以上版本的话还可以使用 `$listeners` 和 `.sync` 这两个属性。
...
对于这种情况可以使用 `Vue2.2` 新增的 `API` `provide` / `inject` ，虽然文档中不推荐直接使用在业务中，但是如果用得好的话还是很有用的。
...
另外需要注意的是 `mixins` 混入的钩子函数会先于组件内的钩子函数执行，并且在遇到同名选项的时候也会有选择性的进行合并，具体可以阅读 [文档](https://v2.cn.vuejs.org/v2/guide/mixins.html)。
...
在 `Vue2.4` 之前都是使用的 `microtasks` ，但是 `microtasks` 的优先级过高，在某些情况下可能会出现比事件冒泡更快的情况，但如果都使用 `macrotasks` 又可能会出现渲染的性能问题。所以在新版本中，会默认使用 `microtasks` ，但在特殊情况下会使用 `macrotasks` ，比如 `v-on`。
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101719152.png)

调用栈过长，再加上中间进行了复杂的操作，就可能导致长时间阻塞主线程，带来不好的用户体验。 `Fiber` 就是为了解决该问题而生。

`Fiber` 本质上是一个虚拟的堆栈帧，新的调度器会按照优先级自由调度这些帧，从而将之前的同步渲染改成了异步渲染，在不影响体验的情况下去分段计算更新。

![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101720646.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101736899.png)

我们可以发现这行代码返回了一个数组，内部包含了相当多的信息，从数据开始在网络中传输到页面加载完成都提供了相应的数据。

![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101736695.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101741930.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101745006.png)
...
- 标识符
  ...
  - `ACK=1`：该字段为一表示确认号字段有效。此外，`TCP` 还规定在连接建立后传送的所有报文段都必须把 `ACK` 置为`1`。
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101748151.png)

在这之前需要了解一个重要的性能指标 `RTT` 。该指标表示发送端发送数据到接收到对端数据所需的往返时间。

#### 建立连接三次握手

![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101749287.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401101755735.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401110941084.png)

发送端窗口是由接收窗口剩余大小决定的。接收方会把当前接收窗口的剩余大小写入应答报文，发送端收到应答后根据该值和当前网络拥塞情况设置发送窗口的大小，所以发送窗口的大小是不断变化的。

当发送端接收到应答报文后，会随之将窗口进行滑动

![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401110941999.png)
...
`TLS` 握手过程如下图：
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111005283.jpg)
...
大家可以通过 该图片 感受下 `HTTP/2` 比 `HTTP/1` 到底快了多少。
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111009942.png)

在 `HTTP/1` 中，因为队头阻塞的原因，你会发现发送请求是长这样的
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111017180.png)

在 `HTTP/2` 中，因为可以复用同一个 `TCP` 连接，你会发现发送请求是长这样的
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111018907.png)

### 2. 二进制传输

`HTTP/2` 中所有加强性能的核心点在于此。在之前的 `HTTP` 版本中，我们是通过文本的方式传输数据。在` HTTP/2` 中引入了新的编码机制，所有传输的数据都会被分割，并采用二进制格式编码。
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111018890.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111022111.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111036342.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111040881.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111042828.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111044405.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111044933.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111049691.jpg)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111051167.jpg)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111053751.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111055712.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111103191.gif)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111104295.gif)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111105231.gif)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111106917.gif)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111107372.gif)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111114682.gif)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111119195.png)
...
![](https://cdn.jsdelivr.net/gh/raisew/gallery/wedoc/202401111123859.png)
...
树的最大深度：该题目来自 [Leetcode](https%3A%2F%2Fleetcode.com%2Fproblems%2Fmaximum-depth-of-binary-tree%2Fdescription%2F) ，题目需要求出一颗二叉树的最大深度
...
[react 精髓之一—diff 算法](https://segmentfault.com/a/1190000004913592)
[js 实现螺旋矩阵算法](https://segmentfault.com/a/1190000008563540)
[JavaScript 算法题之–随机数的生成](https://segmentfault.com/a/1190000002459277)
[JavaScript 算法详解——快速排序](https://segmentfault.com/a/1190000000669157)
...
[10 个 HTML 和 CSS 必须知道的重点难点问题](https://funteas.com/topic/5a930dc8076d14fe50a8e3dd)
