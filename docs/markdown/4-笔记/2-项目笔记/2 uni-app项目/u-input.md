	...
	<view class="u-input" :class="{
		...
		<view class="u-input__right-icon u-flex">
			<view class="u-input__right-icon__clear u-input__right-icon__item" @tap="onClear"
				...
				<u-icon size="40" name="close-circle-fill" color="#c0c4cc" />
			</view>
			<view class="u-input__right-icon__clear u-input__right-icon__item"
				v-if="passwordIcon && type == 'password'">
				<u-icon size="40" :name="!showPassword ? 'eye' : 'eye-fill'" color="#c0c4cc"
...
<script>
	...
	export default {
		...
		watch: {
			...
			defaultValue: {
				handler(nVal, oVal) {
					if (this.type != 'number' && this.type != 'digit') return nVal;
					setTimeout(() => {
						this.defaultValue = this.limitDecimal(this.defaultValue)
					}, 0)
				},
				deep: true,
			}
		...
		methods: {
			...
			handleInput(event) {
				...
				// vue 原生的方法 return 出去
				this.$emit("input", this.limitDecimal(value));
				this.$emit("update:modelValue", this.limitDecimal(value));
				// 当前model 赋值
				...
				setTimeout(() => {
					...
					this.lastValue = value.match(/^\d*(\.?\d{0,2})/g)[0];
					// #endif
